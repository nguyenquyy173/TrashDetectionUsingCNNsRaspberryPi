...
pic_HMI = {'banana':2, 'battery':13, 'coc':7, 'cucumberpeel':4,'egg':5, 'hopxop':12, 'leaves':11, 'lotCoc':8, 'mask':10, 'orange':3,
				'paper':9, 'pen':6, 'bottle':0, 'bag':1}
				
label_HMI = {'banana':2, 'battery':3, 'coc':1, 'cucumberpeel':2,'egg':2, 'hopxop':1, 'leaves':2, 'lotCoc':1, 'mask':1, 'orange':2,
				'paper':2, 'pen':1, 'bottle':0, 'bag':1, 'tai_che':0, 'huu_co':2, 'ko_tai_che':1, 'pin':3}
		
def set_up():
	screen.AddressLW(10,4)
	screen.AddressLW(8,2)
	screen.AddressWriteLB()
	kien.set_angle_servo(18, 90)
...
def scoreLess50():
	...
	# get option
	while tai_che == -1 and ko_tai_che == -1 and huu_co == -1 and pin == -1:
		if screen.AddressReadLB(3)>0: kien.trashCan(1)
		if screen.AddressReadLB(0)>0: kien.trashCan(2)
		if screen.AddressReadLB(1)>0: kien.trashCan(3)
		if screen.AddressReadLB(2)>0: kien.trashCan(4)
...
if __name__ == '__main__':
	...
	while True:
		...
		while distance1 < 1 or distance2 < 1:
